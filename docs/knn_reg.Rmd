---
title: "Algoritmo KNN para regresión"
output: 
  pdf_document:
...
```{r}
xp3 = c(0,0,0)
xp3[1] = (40 - min(d3$long_pico))/(max(d3$long_pico) - min(d3$long_pico))
xp3[2] = (18 - min(d3$prof_pico))/(max(d3$prof_pico) - min(d3$prof_pico))
xp3[3] = (210 - min(d3$long_aleta))/(max(d3$long_aleta) - min(d3$long_aleta))
xp3
```
